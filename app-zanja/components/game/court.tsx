...
import {ArrowLeft,ArrowRight,SlidersHorizontal,Check,Flag,Clock,Gavel,Zap,Trophy,Share2,BookOpen,ChevronRight,RotateCcw,Link2,LoaderCircle,HeartHandshake} from 'lucide-react';
...
type Props={current?:Case;cases:Case[];filter:string;loading:boolean;busy:boolean;celebrate:boolean;onFilter:(s:string)=>void;onBack:()=>void;onVote:(side:'a'|'both'|'b')=>Promise<boolean>;onNext:()=>void;onCreate:()=>void;onReport:()=>void;onShare:(c:Case,invite?:boolean)=>void;onRevise:(c:Case)=>void;};
...
export function Court({current:c,cases,filter,loading,busy,celebrate,onFilter,onBack,onVote,onNext,onCreate,onReport,onShare,onRevise}:Props){
 const [filters,setFilters]=useState(false),[choice,setChoice]=useState(filter),[argumentsVisible,setArgumentsVisible]=useState(false),[pressed,setPressed]=useState<'a'|'both'|'b'|null>(null);
 ...
 const choose=async(side:'a'|'both'|'b')=>{if(!canVote||pressed)return;setPressed(side);if(!await onVote(side))setPressed(null);};
 return <section className={'court-screen '+(result?'court-results':'')}>
  ...
  {!c?<div className="court-empty"><Gavel size={48}/><h2>{loading?'Preparando el duelo…':'¡Todo zanjado!'}</h2><p>No quedan casos en esta selección. Elige otro tema o crea una zanja.</p><button className="game-btn yellow" onClick={()=>onFilter('Todas')}><RotateCcw size={19}/>VER TODOS</button><button className="quiet-btn" onClick={onCreate}>Crear una zanja<ArrowRight size={18}/></button></div>:result?<div className={'court-result-body '+(celebrate?'result-arrives':'')} key={'result-'+c.id}>
   ...
   <div className="verdict-scorecards">{(['a','both','b'] as const).map(side=>{const percent=c.total?Math.round((c.counts?.[side]||0)/c.total*100):0;return <div className={'verdict-card verdict-card-'+side+(c.choice===side?' is-choice':'')} key={side}><div className="verdict-card-top"><h3>{side==='both'?'AMBOS':<>BANDO <b>{side.toUpperCase()}</b></>}</h3><strong>{percent}<span>%</span></strong></div><Progress value={percent} aria-label={side==='both'?'Ambos':`Bando ${side.toUpperCase()}`}/><div className="verdict-card-foot"><span>{c.counts?.[side]||0} {(c.counts?.[side]||0)===1?'voto':'votos'}</span>{c.choice===side&&<span className="your-choice"><Check size={13}/>TU VOTO</span>}</div></div>;})}</div>
