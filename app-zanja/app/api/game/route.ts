...
export async function GET(req:Request){try{
 ...
 const vote:any=personal.results.find((v:any)=>v.case_id===c.id);const closed=c.closes>0&&c.closes<=now&&c.status!=='waiting';const counts={a:0,both:0,b:0};vs.results.filter((v:any)=>v.case_id===c.id).forEach((v:any)=>{counts[v.choice as keyof typeof counts]=Number(v.n);});
...
export async function POST(req:Request){try{
 ...
 if(data.action==='vote'){
 if(!['a','both','b'].includes(data.choice))return fail('Elige una respuesta válida.');if(c.owner===user||c.respondent===user)return fail('Los protagonistas no votan su propio caso.',403);
